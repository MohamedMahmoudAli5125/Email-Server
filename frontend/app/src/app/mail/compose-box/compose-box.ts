...
export class ComposeBox {
  ...
  ngOnChanges(changes: SimpleChanges) {
    ...
    if (changes['currentEmail']?.currentValue) {
      ...
      if (email.id) {
        this.isDraft = true;
        this.buttonPlaceholder = 'Update Draft';
      } else {
        this.isDraft = false;
        this.buttonPlaceholder = 'Draft';
      }
  ...
  sendEmail(emailForm: NgForm) {
    ...
    if (this.isDraft) {
      this.mailService.sendDraft(formData, this.composeEmail.id).subscribe({
        next: (res) => {
          alert('Email sent successuflly');
          this.onCloseCompose();
        },
        error: (err) => {
          console.log(err)
  ...
  draftEmail(emailForm: NgForm) {
    ...
    if (this.isDraft) {
      this.mailService.updateDraft(formData, this.composeEmail.id).subscribe({
        next: (res) => {
          alert('Draft has been updated');
          this.onCloseCompose();
        },
        error: (err) => {
          console.log(err)
